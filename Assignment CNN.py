...
#%% define the model / references:(https://keras.io/api/layers/convolution_layers/convolution1d/
#https://keras.io/api/layers/pooling_layers/max_pooling1d/ and https://keras.io/api/layers/recurrent_layers/lstm/)
def model_fn(params):
    model = Sequential()
...
#%% define the search space that will be parsed with the random search
search_space = {
    "filters": [32, 64, 128],
    "kernel_size": [2, 3, 4],
    "lstm_units": [30, 50, 70],
...
def random_search(model_fn, search_space, n_iter, search_dir): 
    ...
    for i in range(n_iter):
        ...
        #extract best epoch in terms of validation accuracy
        ...
        #we need this to clear session and free up resources
        tf.keras.backend.clear_session()
        print(f"iteration {i + 1} – {', '.join(f'{k}:{v:.4g}' for k, v in result.items())}")
    #load best model
    best_model = tf.keras.models.load_model(best_model_path)
    #convert to df and save in our directory
    results_df = pd.DataFrame(results)
    results_df.to_csv(results_path)

    return results_df, best_model
#%%Run training
results, best_model = random_search(model_fn, search_space, n_iter=100, search_dir="model_search")
...
#%% check irregularities (maybe all are just positive)

# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 18:14:04 2023
...
#%% Step 7
def lstm_model_fn(params):
    lstm_model = Sequential()
   
   # Input goes directly into LSTM layers
    lstm_model.add(LSTM(params["lstm_units"], return_sequences=True, input_shape=(14, 2)))
    lstm_model.add(LSTM(params["lstm_units"]))
    lstm_model.add(Dropout(params["dropout_rate"]))
   
    lstm_model.add(Dense(1, activation="sigmoid"))  # Binary classification

    lstm_model.compile(optimizer=tf.keras.optimizers.Adam(params["learning_rate"]), 
                       loss="binary_crossentropy", 
                       metrics=["accuracy"])
   
   # Training the lstm_model
    history = lstm_model.fit(train_sequences, train_labels, 
...
#%%
def random_search(lstm_model_fn, search_space_lstm, n_iter, search_dir): 
    results = []  # initialise an empty set 

    if not os.path.exists(search_dir):
        os.mkdir(search_dir)  # use os and create a directory if it doesn"t exist

    best_model_lstm_path = os.path.join(search_dir, "best_model_lstm.h5")
    results_path = os.path.join(search_dir, "results_lstm.csv")

    best_val_acc = 0  # Initialize best validation accuracy to 0

    for i in range(n_iter):
        params = {k: v[np.random.randint(len(v))] for k, v in search_space_lstm.items()}

        history, lstm_model = lstm_model_fn(params)
        ...
        if result["val_accuracy"] > best_val_acc:
            ...
            lstm_model.save(best_model_lstm_path)
    ...
    best_lstm_model = tf.keras.models.load_model(best_model_lstm_path)
    results_df = pd.DataFrame(results)
    results_df.to_csv(results_path)

    return results_df, best_lstm_model
#%%Step 9
results, best_lstm_model = random_search(lstm_model_fn, search_space_lstm, n_iter=50, search_dir="model_search")

#%% 
lstm_model = load_model("C:/Users/User/OneDrive/Dokumente/Uni/12. Big Data and Deep Learning/Data/model_search/best_model_lstm.h5")

#%% Evaluate the training data
train_loss, train_accuracy = lstm_model.evaluate(train_sequences, train_labels)
print(f"Training Accuracy: {train_accuracy*100:.2f}%")
#%% Evaluate on the validation data
val_loss, val_accuracy = lstm_model.evaluate(val_sequences, val_labels)
